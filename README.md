...
jobs 정보 시스템 내용
---
[1]+첫 번째 열의 대괄호 안의 숫자는 프로세스 ID가 된다. 이 번호는 현재 쉘에서 유효하다.
...
### kill
    프로세스에 신호를 보내는 명령어
리눅 시스템에서kill명령어는 프로세스에 신호를 보내어 그 프로세스를 종료하거나 다시 시작하는 데 사용된다.
시스템 리소스를 과도하게 사용하거나 응답하지 않는 프로세스를 안전하게 종료하는 데 유용하다.
일반적으로 해당 Process에 문제가 있어서 다시 시작하고자 할 때에는 SIGHUP, SIGTERM, SIGKILL의 시그널 순서로 시도해보길 권장된다.<br>
*시그널이란 인터럽트의 일종으로 어떤 이벤트의 발생을 프로세스에게 알려주는 것이다.*

##### 사용법
      kill [옵션] <PID>
1. 먼저, ps -ef 명령어를 사용하여 종료하려는 프로세스의 ID를 확인한다.<br>
2. 그 후, kill -15 [프로세스ID] 명령어로 프로세스에 안전 종료 신호를 보낸다.<br>
3. 만약 프로세스가 이 신호에 반응하지 않는 경우, kill -9 [프로세스ID] 명령어를 사용하여 강제로 종료할 수 있다.<br>

사용 옵션
---
    -l           : 모든 시그널 목록을 표시
    -s           : 지정한 시그널을 보냄
    -9           : IGKILL 시그널을 보내어 강제 종료
